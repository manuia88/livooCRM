...
export interface AdvancedFiltersState {
  ...
  construction_m2_min: number | null
  construction_m2_max: number | null
  land_m2_min: number | null
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => update('parking_spaces', null)}
                className={cn(
                  'px-4 py-2 rounded-xl text-sm font-medium transition-colors',
                  filters.parking_spaces === null ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                )}
              >
                Cualquiera
              </button>
              {COUNT_OPTIONS.map((n) => (
                <button
                  key={n}
                  type="button"
                  onClick={() => update('parking_spaces', n)}
                  className={cn(
                    'px-4 py-2 rounded-xl text-sm font-medium transition-colors',
                    filters.parking_spaces === n ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  )}
                >
                  {n}+
                </button>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  min={0}
                  placeholder="0"
                  value={filters.construction_m2_min ?? ''}
                  onChange={(e) => update('construction_m2_min', e.target.value ? Number(e.target.value) : null)}
              ...
              <div>
                ...
                <Input
                  ...
                  min={0}
                  placeholder="—"
                  value={filters.construction_m2_max ?? ''}
                  onChange={(e) => update('construction_m2_max', e.target.value ? Number(e.target.value) : null)}
                  className="rounded-xl mt-1"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  min={0}
                  placeholder="0"
                  value={filters.land_m2_min ?? ''}
                  onChange={(e) => update('land_m2_min', e.target.value ? Number(e.target.value) : null)}
                  className="rounded-xl mt-1"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  min={0}
                  placeholder="—"
                  value={filters.land_m2_max ?? ''}
                  onChange={(e) => update('land_m2_max', e.target.value ? Number(e.target.value) : null)}
                  className="rounded-xl mt-1"
                />
              </div>
            </div>
